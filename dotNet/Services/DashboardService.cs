...
namespace Sabio.Services
{
    public class DashboardService : IDashboardService
    {
        ...
        public AdminDashboard GetDataForDashboard()
        {
            ...
            _data.ExecuteCmd(
                ...
                delegate (IDataReader reader, short set)
                {
                    ...
                    {
                        BaseLender aLender = MapSingleLender(reader, ref startingIndex);

                        if (lenders == null)
                        {
                            lenders = new List<BaseLender>();
                        }
                        lenders.Add(aLender);
                    }
                    else if (set == 2)
        ...
        public List<BaseLender> QueryLenders(string query)
        {
            List<BaseLender> lenders = null;
        ...
        private static BaseLender MapSingleLender(IDataReader reader, ref int startingIndex)
        {
            BaseLender aLender = new BaseLender();
            aLender.Name = reader.GetSafeString(startingIndex++);
            aLender.Logo = reader.GetSafeString(startingIndex++);
            aLender.Website = reader.GetSafeString(startingIndex++);
            aLender.StatusId = reader.GetSafeInt32(startingIndex++);
            aLender.CreatedBy = reader.GetSafeInt32(startingIndex++);
            aLender.ModifiedBy = reader.GetSafeInt32(startingIndex++);
            return aLender;
        }

        private static AdminDashboard MapSingleAdmin(IDataReader reader, ref int startingIndex)
        {
